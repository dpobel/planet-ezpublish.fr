<section class="about">
<h1 class="title-page">{'eZ Publish information: %version'|i18n( 'design/standard/ezinfo/about',, hash( '%version', $ezinfo ) )|wash}</h1>
                    ...
                    <li>
                        {foreach $extension as $ext_key => $ext_info}
                            ...
                            {if not( is_array( $ext_info ) )}
                                {$ext_info|autolink}<br />
                            {else}
                                <ul>
                                    <li>
                                        {foreach $ext_info as $ext_info_key => $ext_info_value}
                                            {$ext_info_key|wash} : {$ext_info_value|wash|autolink}<br />
                                        {/foreach}
                                    </li>
                                </ul>
                            {/if}
